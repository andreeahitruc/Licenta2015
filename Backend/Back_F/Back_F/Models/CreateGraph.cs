...
namespace Back_F.Models
{
    ...
    {        
        public static void getList(Flickr f, List<linkfriend> list_of_friends, string userId)
        {
            ...
            foreach (var user in list_of_friends)
            {
               // FlickrNet.PhotoCollection photoCollection = f.PeopleGetPublicPhotos(user.UserId);
                tags = f.TagsGetListUser(user.IdFriend).GroupBy(x => x.TagName).OrderByDescending(g => g.Count()).Take(5).Select(gg => gg.Key).ToList();
